...
import sys
sys.path.append('')
from tools.data_converter.vector_map_corridor import CorridorConstructor
...
def create_nuscenes_infos(root_path,
    ...
    corridor_constructor = CorridorConstructor(root_path, MAPS)
    ...
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(
        nusc, nusc_can_bus, corridor_constructor, train_scenes, val_scenes, test, max_sweeps=max_sweeps)
...
def _fill_trainval_infos(nusc,
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        if not test:
            ...
            #maps
            map_anns, vector_map = corridor_constructor.obtain_vectormap(info, point_cloud_range)
            info["gt_maps"] = map_anns
            corridors = corridor_constructor.obtain_corridor(info, nusc, vector_map, sample)
            info['gt_corridor'] = corridors
