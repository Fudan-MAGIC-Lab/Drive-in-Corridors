...
class PlanningMetric():
    ...
    def _get_poly_region_in_image(self,param):
        lidar2cv_rot = np.array([[1,0], [0,1]])  #try
    ...
    def evaluate_single_coll(self, traj, segmentation, input_gt, gt_traj=None, index=None):
        ...
        for i in range(traj.size(0)):
            ...
            rotate_angle = yaw0 + cur_rotate_angle
    ...
    def debug(self, traj, segmentation, map_segmentation):
        ...
        # axes.axis('off')
        collision1 = np.full(n_future, False)
        collision2 = np.full(n_future, False)
        for t in range(n_future):
            ...
            axes.grid(False)
            axes.invert_yaxis()
            plt.savefig('vis/segmentation_with_trajectory'+str(t)+'.png')

            I = np.logical_and(
                np.logical_and(rr >= 0, rr < self.bev_dimension[0]),
                np.logical_and(cc >= 0, cc < self.bev_dimension[1]),
            )
            collision1[t] = np.any(segmentation[t,  cc[I], rr[I]].cpu().numpy())
            collision2[t] = np.any(map_segmentation[t,  cc[I], rr[I]].cpu().numpy())
                    
        return torch.from_numpy(collision1).to(device=traj.device), torch.from_numpy(collision2).to(device=traj.device)
    ...
    def evaluate_coll(
        ...
        for i in range(B):
            ...
            gt_box_coll = gt_box_coll.to(trajs.device)
