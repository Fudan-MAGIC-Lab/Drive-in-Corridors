...
# from projects.DifferentiableLQR.QPControl_soft_corner_12N_learned_weights import QPControl  # soft constraint
from projects.DifferentiableLQR.QPControl_HARD_corner_12N_learned_weights import QPControl
from projects.DifferentiableLQR.CorridorClipper import CorridorClipper
...
class VADHead(DETRHead):
    ...
    def __init__(self,
                ...
                loss_map_cls=dict(
                 ...
                 loss_corridor_reg=dict(type='L1Loss', loss_weight=1.0),
                 loss_corridor_bound=dict(type='CorridorMapBoundLoss', loss_weight=0.1),
                 loss_corridor_col=dict(type='CorridorColLoss', loss_weight=0.1),
                 loss_corridor_area=dict(type='CorridorAreaLoss', loss_weight=0.1),
                 loss_mpc_l1=dict(type='L1Loss', loss_weight=1.0), 
                 ...
                 valid_fut_ts=6,
                #  cor_pts = 5, 
                 cor_pts = 6, # dim 5 x y sin_yaw cos_yaw l w
                 clip=False,
                 stage_1=False,
                 **kwargs):
        ...
        self.cor_pts = cor_pts

        self.stage_1 = stage_1
        self.qp_fail_time = 0
        self.qp_total_time = 0
        ...
        self.loss_corridor_reg = build_loss(loss_corridor_reg)
        self.loss_corridor_bound = build_loss(loss_corridor_bound)
        self.loss_corridor_col = build_loss(loss_corridor_col)
        self.loss_corridor_area = build_loss(loss_corridor_area)

        if not self.stage_1:
            self.loss_plan_mpc = build_loss(loss_mpc_l1)
            self.qp = QPControl()
            self.clipper = CorridorClipper()
    ...
    def _init_layers(self):
        ...
        ego_cor_decoder = []
        ego_cor_dec_in_dim = self.embed_dims*2 + len(self.ego_lcf_feat_idx) \
            if self.ego_lcf_feat_idx is not None else self.embed_dims*2
        for _ in range(self.num_reg_fcs):
            ego_cor_decoder.append(Linear(ego_cor_dec_in_dim, ego_cor_dec_in_dim))
            ego_cor_decoder.append(nn.ReLU())
        ego_cor_decoder.append(Linear(ego_cor_dec_in_dim, self.ego_fut_mode*self.cor_pts * self.fut_ts))
        self.ego_cor_decoder = nn.Sequential(*ego_cor_decoder)
        ...
        ego_cor_dec_in_dim = self.embed_dims*2 + len(self.ego_lcf_feat_idx) \
            if self.ego_lcf_feat_idx is not None else self.embed_dims*2
        for _ in range(self.num_reg_fcs):
            ego_cor_bias.append(Linear(ego_cor_dec_in_dim, ego_cor_dec_in_dim))
            ego_cor_bias.append(nn.ReLU())
        ego_cor_bias.append(Linear(ego_cor_dec_in_dim, self.ego_fut_mode* self.n_bias * self.fut_ts))
        self.ego_cor_bias = nn.Sequential(*ego_cor_bias)
        '''
    ...
    def forward(self,
        ...
        # corridor
        outputs_ego_cors = self.ego_cor_decoder(ego_feats) 
        outputs_ego_cors = outputs_ego_cors.reshape(outputs_ego_cors.shape[0], 
                                                    self.ego_fut_mode, self.fut_ts, self.cor_pts)
        outputs_ego_cors[..., 0:2] = outputs_ego_cors[..., 0:2].sigmoid() # cx, cy
        outputs_ego_cors[..., 4:6] = outputs_ego_cors[..., 4:6].sigmoid() # w, l
        # outputs_ego_cors = outputs_ego_cors.sigmoid() # w, l  # this for epoch_normlized_12.pth!!!

        # corridor refine
        # outputs_cors_bias = self.ego_cor_bias(ego_feats)
        # outputs_cors_bias = outputs_cors_bias.reshape(outputs_ego_cors.shape[0], 
        #                                             self.ego_fut_mode, self.fut_ts, self.n_bias)
        # outputs_cors_bias[..., 0:1] = outputs_cors_bias[..., 0:1].sigmoid() * 10.0 -5.0  # cx
        # outputs_cors_bias[..., 1:2] = outputs_cors_bias[..., 1:2] * 20 - 10.0 # cy
        # outputs_cors_bias[..., 2:3] = outputs_cors_bias[..., 2:3].exp() # 
        # outputs_cors_bias[..., 3:4] = outputs_cors_bias[..., 3:4].exp()



        # '''
        # QP optimization
        idx = (ego_fut_cmd == 1).squeeze(1).squeeze(1) #  [B, fut_mode]
        outputs_ego_cors_mpc = to_real_corridor(outputs_ego_cors, self.pc_range)[idx]  #用來做優化, [B, 6, 5]
        outputs_ego_trajs_mpc = outputs_ego_trajs[idx]  # without bias [B, 6, 2]
        gt_corridors = gt_corridor[0].to(outputs_ego_cors_mpc.dtype)
        canbus = img_metas[0]['can_bus']
        ...
        if self.stage_1:
            outs = {
                ...
                "ego_cor_preds": outputs_ego_cors,
            }

        else:
            # # check bug
            # qp_traj = self.qp(canbus, ego_lcf_feat, outputs_ego_trajs_mpc.detach(), outputs_ego_cors_mpc.detach(), train=True)  # training weights
            # outputs_mpc_traj = qp_traj
            is_testing = not self.training
            if self.clip and is_testing:
                with torch.no_grad():
                    corridors = outputs_ego_cors_mpc
                    traj = outputs_ego_trajs_mpc
                    lane_preds = map_outputs_pts_coords[-1]
                    lane_score_preds = map_outputs_classes[-1].sigmoid()
                    agent_preds = outputs_coords[-1]
                    all_traj_preds = outputs_trajs.repeat(outputs_coords.shape[0], 1, 1, 1, 1)
                    ...
                    agent_score_preds = outputs_classes[-1].sigmoid()
                    all_traj_cls_scores = outputs_trajs_classes.repeat(outputs_coords.shape[0], 1, 1, 1)
                    agent_fut_cls_preds = all_traj_cls_scores[-1].view(batch, num_agent, self.fut_mode).sigmoid()
                    clipped_cor = self.clipper.ProcessCorridors(corridors, 
                                                                traj, 
                                                                lane_preds, 
                                                                lane_score_preds, 
                                                                agent_preds,
            ...
            self.qp_total_time += 1
            try:
                # qp_traj = self.qp.solve(canbus, ego_lcf_feat, outputs_ego_trajs_mpc, gt_corridors, train=True)  # gt corridor
                qp_traj = self.qp(canbus, ego_lcf_feat, outputs_ego_trajs_mpc.detach(), outputs_ego_cors_mpc.detach(), train=False)  # training weights
                # qp_traj = self.qp(canbus, ego_lcf_feat, outputs_ego_trajs_mpc.detach(), clipped_cor, train=False)  # clip corridor
            except:
                self.qp_fail_time += 1
                # print("QP solver failure!", self.qp_fail_time/self.qp_total_time*100, '%')
                outputs_mpc_traj = outputs_ego_trajs_mpc
                # outputs_mpc_traj = None  # no grad when solver fails
            else:
                outputs_mpc_traj = qp_traj
                # outputs_mpc_traj = outputs_mpc_traj.unsqueeze(1).repeat(1,self.ego_fut_mode,1,1) # fake repeat, [B, self.ego_fut_mode, T, 2] # no grad when solver fails


            outputs_mpc_traj = outputs_mpc_traj.unsqueeze(1).repeat(1,self.ego_fut_mode,1,1) # fake repeat, [B, self.ego_fut_mode, T, 2]
            #'''
            ...
            outs = {
                ...
                'ego_fut_preds': outputs_ego_trajs,
                "ego_cor_preds": outputs_ego_cors,
                "mpc_trajs": outputs_mpc_traj,
                "clipped_cor": clipped_cor,
            }
    ...
    def loss_corridor(self,
        ...
        """"Loss function for ego vehicle planning.
        Args:
            ego_cor_preds (Tensor): [B, ego_fut_mode, fut_ts, 6]
            gt_corridor (Tensor): [B, fut_ts, 5]
            ego_fut_masks (Tensor): [B, fut_ts]
            ego_fut_cmd (Tensor): [B, ego_fut_mode]
            lane_preds (Tensor): [B, num_vec, num_pts, 2]
            lane_score_preds (Tensor): [B, num_vec, 3]
            ...
            agent_fut_cls_scores (Tensor): [B, num_agent, fut_mode]
        Returns:
            loss_corridor_reg (Tensor): planning reg loss.
            loss_corridor_bound (Tensor): planning map boundary constraint loss.
            loss_corridor_col (Tensor): planning col constraint loss.
            loss_corridor_area (Tensor): Corridor as large as possible
        """

        gt_corridor = gt_corridor.unsqueeze(1).repeat(1, self.ego_fut_mode, 1, 1)
        gt_corridor = normalize_2d_bbox_corridor(gt_corridor, self.pc_range)

        loss_plan_l1_weight = ego_fut_cmd[..., None, None] * ego_fut_masks[:, None, :, None]
        loss_plan_l1_weight = loss_plan_l1_weight.repeat(1, 1, 1, self.cor_pts)
        outputs_ego_cors = denormalize_2d_bbox_corridor(ego_cor_preds, self.pc_range)  #用來算loss

        loss_corridor_l1 = self.loss_corridor_reg(
            outputs_ego_cors,
            gt_corridor,
            loss_plan_l1_weight
        )
        ...
        ego_cor_preds = to_real_corridor(ego_cor_preds, self.pc_range)
        # detached_ego_cor_preds = torch.cat([ego_cor_preds[..., :3], ego_cor_preds[..., 3:].detach()], dim=-1)

        loss_corridor_bound = self.loss_corridor_bound(
            ego_cor_preds[ego_fut_cmd==1],
            lane_preds,
            lane_score_preds,
            weight=ego_fut_masks
        )

        loss_corridor_col = self.loss_corridor_col(
            ego_cor_preds[ego_fut_cmd==1],
            agent_preds,
            agent_fut_preds,
            agent_score_preds,
        ...
        loss_corridor_area = self.loss_corridor_area(
            ego_cor_preds[ego_fut_cmd==1],
            weight=ego_fut_masks
        )
        ...
        if digit_version(TORCH_VERSION) >= digit_version('1.8'):
            loss_corridor_l1 = torch.nan_to_num(loss_corridor_l1)
            loss_corridor_bound = torch.nan_to_num(loss_corridor_bound)
            loss_corridor_col = torch.nan_to_num(loss_corridor_col)
            loss_corridor_area = torch.nan_to_num(loss_corridor_area)
        
        loss_plan_dict = dict()
        loss_plan_dict['loss_corridor_l1'] = loss_corridor_l1
        loss_plan_dict['loss_corridor_bound'] = loss_corridor_bound
        loss_plan_dict['loss_corridor_col'] = loss_corridor_col
        loss_plan_dict['loss_corridor_area'] = loss_corridor_area
    ...
    def loss(self,
        ...
        ego_cor_preds = preds_dicts['ego_cor_preds']
        ...
        # grads = torch.autograd.grad(outputs=loss_planning_dict['loss_plan_reg'], inputs=[ego_fut_preds], retain_graph=True, allow_unused=True)
        # print(grads)
        ...
        # Corridor Loss
        loss_corridor_input = [ego_cor_preds, gt_corridor, ego_fut_masks, ego_fut_cmd,
                           map_all_pts_preds[-1], map_all_cls_scores[-1].sigmoid(),
                        #    all_bbox_preds[-1][..., 0:2], agent_fut_preds,
                           all_bbox_preds[-1], agent_fut_preds,
                           all_cls_scores[-1].sigmoid(), agent_fut_cls_preds.sigmoid()]
        loss_planning_dict = self.loss_corridor(*loss_corridor_input)
        loss_dict["loss_cor_pts"] = loss_planning_dict['loss_corridor_l1']
        loss_dict["loss_cor_bound"] = loss_planning_dict['loss_corridor_bound']
        loss_dict["loss_cor_col"] = loss_planning_dict['loss_corridor_col']
        loss_dict["loss_cor_area"] = loss_planning_dict['loss_corridor_area']
        
        if self.stage_1:
            pass
        else:
            ego_fut_mpc = preds_dicts['mpc_trajs']
            ego_fut_gt = ego_fut_gt.unsqueeze(1).repeat(1, self.ego_fut_mode, 1, 1)
            loss_plan_l1_weight = ego_fut_cmd[..., None, None] * ego_fut_masks[:, None, :, None]
            loss_plan_l1_weight = loss_plan_l1_weight.repeat(1, 1, 1, 2)
            loss_final_plan_l1 = self.loss_plan_mpc(
                ego_fut_mpc,
                ego_fut_gt,
                loss_plan_l1_weight
            )
            loss_dict['loss_mpc_l1'] = loss_final_plan_l1


        '''
            grads_traj = torch.autograd.grad(outputs=loss_final_plan_l1, inputs=[ego_fut_preds], retain_graph=True, allow_unused=True)
            print(grads_traj[0][torch.where(ego_fut_cmd)])
            grads_cor = torch.autograd.grad(outputs=loss_final_plan_l1, inputs=[ego_cor_preds], retain_graph=True, allow_unused=True)
            grads_cor[0][torch.where(ego_fut_cmd)]

            grads_traj = torch.autograd.grad(outputs=loss_final_plan_l1, inputs=[ego_fut_preds], retain_graph=True, allow_unused=True)
            grads_traj_norm = grads_traj[0][torch.where(ego_fut_cmd)].norm()
            # print('grads_traj_norm', grads_traj[0][torch.where(ego_fut_cmd)])
            grads_cor = torch.autograd.grad(outputs=loss_final_plan_l1, inputs=[ego_cor_preds], retain_graph=True, allow_unused=True)
            grads_cor_norm = grads_cor[0][torch.where(ego_fut_cmd)].norm()
            # print('grads_cor_norm', grads_cor[0][torch.where(ego_fut_cmd)])
            grads_weights = torch.autograd.grad(outputs=loss_final_plan_l1, inputs=[self.qp.weight], retain_graph=True, allow_unused=True)
            # print('grads_weights_norm', grads_weights[0].norm())
            grads_weights_norm = grads_weights[0].norm()

            # Record gradients norm for visualization
            if not hasattr(self, 'grad_norms_history'):
            ...
            # Append current gradient norms to history
            self.grad_norms_history['traj'].append(float(grads_traj_norm))
            self.grad_norms_history['corridor'].append(float(torch.log(grads_cor_norm+1e-20)))
            self.grad_norms_history['weights'].append(float(grads_weights_norm))
            
            # Periodically save or visualize the gradient norms (every 100 iterations)
            if len(self.grad_norms_history['traj']) % 1 == 0:
                try:
                    import matplotlib.pyplot as plt
                    import numpy as np
                    import os
                    ...
                    os.makedirs('grad_norm_plots', exist_ok=True)
                    
                    # Convert lists to numpy arrays for plotting
                    iterations = np.arange(len(self.grad_norms_history['traj']))
                    traj_norms = np.array(self.grad_norms_history['traj'])
                    corridor_norms = np.array(self.grad_norms_history['corridor'])
                    weights_norms = np.array(self.grad_norms_history['weights'])
                    
                    # Create plot with three subfigures
                    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(10, 12))
                    fig.suptitle('Gradient Norms Over Training')

                    # Trajectory gradients
                    ax1.plot(iterations, traj_norms, 'b-')
                    ax1.set_ylabel('Trajectory\nGradient Norm')
                    ax1.grid(True)

                    # Corridor gradients
                    ax2.plot(iterations, corridor_norms, 'g-')
                    ax2.set_ylabel('Corridor\nGradient LogNorm')
                    ax2.grid(True)

                    # Weights gradients
                    ax3.plot(iterations, weights_norms, 'r-')
                    ...
                    # Save plot
                    plt.savefig(f'grad_norm_plots/grad_norms_iter.png')
                    plt.close()
                    
                    # Also save the raw data for future analysis
                    np.savez(
                        'grad_norm_plots/grad_norms_data.npz',
                        traj_norms=traj_norms,
                        corridor_norms=corridor_norms,
    ...
    def get_bboxes(self, preds_dicts, img_metas, rescale=False):
        ...
        for i in range(num_samples):
            ...
            corr_pts = map_preds['corridor_pts']
            corr_clip = map_preds['corridor_clip']

            ret_list.append([bboxes, scores, labels, trajs, map_bboxes,
                             map_scores, map_labels, map_pts, corr_pts, corr_clip])
