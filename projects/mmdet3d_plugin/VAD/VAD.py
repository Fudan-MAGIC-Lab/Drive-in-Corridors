...
from projects.mmdet3d_plugin.VAD.planner.metric_bp import PlanningMetric
...
class VAD(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        self.vad_metric = None

        if freeze:
            for param in self.parameters():
                param.requires_grad = False
                # unfreeze
            training_layers = [self.pts_bbox_head.ego_query, 
                                self.pts_bbox_head.ego_agent_pos_mlp, 
                                self.pts_bbox_head.agent_fus_mlp,
                                self.pts_bbox_head.ego_agent_decoder,
                                self.pts_bbox_head.ego_map_pos_mlp,
                                self.pts_bbox_head.ego_map_decoder,
                                self.pts_bbox_head.ego_fut_decoder,
                                self.pts_bbox_head.ego_cor_decoder,
                                self.pts_bbox_head.qp
                                ]
    ...
    def forward_pts_train(self,
        ...
        outs = self.pts_bbox_head(pts_feats, img_metas, prev_bev,
                                  ego_his_trajs=ego_his_trajs, ego_lcf_feat=ego_lcf_feat, gt_corridor=gt_corridor, ego_fut_trajs=ego_fut_trajs, ego_fut_cmd=ego_fut_cmd)
        loss_inputs = [
            gt_bboxes_3d, gt_labels_3d, map_gt_bboxes_3d, map_gt_labels_3d,
            outs, ego_fut_trajs, ego_fut_masks, ego_fut_cmd, gt_attr_labels, gt_corridor
        ]
    ...
    def simple_test_pts(
        ...
        outs = self.pts_bbox_head(x, img_metas, prev_bev=prev_bev,
                                  ...
                                  gt_corridor=gt_corridor, ego_fut_trajs=ego_fut_trajs, ego_fut_cmd=ego_fut_cmd)
        bbox_list = self.pts_bbox_head.get_bboxes(outs, img_metas, rescale=rescale)

        bbox_results = []
        for i, (bboxes, scores, labels, trajs, map_bboxes, \
                map_scores, map_labels, map_pts, corr_pts, corr_clip) in enumerate(bbox_list):
            ...
            bbox_result['ego_fut_mpc'] = outs['mpc_trajs'][i].cpu()
            bbox_result['ego_fut_cmd'] = ego_fut_cmd.cpu()
            bbox_result['ego_cor_preds'] = corr_pts.cpu()  
            bbox_result['clipped_cor'] = corr_clip.cpu()
        ...
        with torch.no_grad():
            ...
            gt_map_label = map_gt_labels_3d[0]
            ...
            ego_fut_preds = bbox_result['ego_fut_mpc']
            ...
            ref_ego_fut_trajs = bbox_result['ego_fut_preds']
            ref_ego_traj = ref_ego_fut_trajs[ego_fut_cmd_idx]
            ref_ego_traj =ref_ego_traj.cumsum(dim=-2)
            ego_fut_trajs = ego_fut_trajs.cumsum(dim=-2)

            metric_dict_planner_stp3 = self.compute_planner_metric_stp3(
                pred_ego_fut_trajs = ego_fut_pred[None],
                ref_ego_trajs = ref_ego_traj[None],
                gt_ego_fut_trajs = ego_fut_trajs[None],
                gt_agent_boxes = gt_bbox,
                gt_agent_feats = gt_attr_label.unsqueeze(0),
                gt_map_bbox=gt_map_bbox,
                gt_map_label=gt_map_label,
                fut_valid_flag = fut_valid_flag
            )
            metric_dict.update(metric_dict_planner_stp3)
            bbox_results[0]['coll'] = metric_dict_planner_stp3['plan_obj_box_col_3s']
            bbox_results[0]['inter'] = metric_dict_planner_stp3['plan_obj_box_map_intersect_3s']
            # bbox_results[0]['good'] = metric_dict_planner_stp3['good_sample']
    ...
    def compute_planner_metric_stp3(
        self,
        pred_ego_fut_trajs,  # used as mpc traj
        ref_ego_trajs, # used as nn pred
        gt_ego_fut_trajs,
        gt_agent_boxes,
        gt_agent_feats,
        gt_map_bbox,
        ...
        metric_dict = {
            ...
            'plan_obj_map_intersect_1s':0,
            'plan_obj_map_intersect_2s':0,
            'plan_obj_map_intersect_3s':0,
            'plan_obj_box_map_intersect_1s':0,
            'plan_obj_box_map_intersect_2s':0,
            'plan_obj_box_map_intersect_3s':0,
            # 'good_sample': 0,
        }
        ...
        gt_map_segmentations = self.planning_metric.get_map_label(segmentation, gt_map_bbox, gt_map_label)  
        occupancy = occupancy.to()
        '''
        debug_traj = pred_ego_fut_trajs[0].detach().cpu()
        ccc, ccm = self.planning_metric.debug(debug_traj, occupancy[0], gt_map_segmentations[0])
        if torch.any(ccc) or torch.any(ccm):
            print('xxx')
        '''
        ...
        for i in range(future_second):
            if fut_valid_flag:
                ...
                obj_coll, obj_box_coll = self.planning_metric.evaluate_coll(
                    pred_ego_fut_trajs[:, :cur_time].detach(),
                    gt_ego_fut_trajs[:, :cur_time],
                    gt_map_segmentations,
                    ignore_gt=False
                )
                # counting center is meaningless, since boundary only makes up 1 pixel
                metric_dict['plan_obj_map_intersect_{}s'.format(i+1)] = obj_box_coll.mean().item() # obj_coll.mean().item()  
                metric_dict['plan_obj_box_map_intersect_{}s'.format(i+1)] = obj_box_coll.max().item()

                ref_obj_coll, ref_obj_box_coll = self.planning_metric.evaluate_coll(
                    ref_ego_trajs[:, :cur_time].detach(),
                    gt_ego_fut_trajs[:, :cur_time],
                    gt_map_segmentations,
                    ignore_gt=False
                )
                # opt_coll = obj_box_coll.max().item()
                # pred_coll = ref_obj_box_coll.max().item()
                # if pred_coll==1 and opt_coll==0:
                #     metric_dict['good_sample'] = 1
