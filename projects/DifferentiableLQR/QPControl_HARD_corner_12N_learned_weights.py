...
class QPControl(nn.Module):
	...
	def forward(self, canbus, ego_lcf_feat, tracking_traj, corridor_rect, train=False):
		'''
		...
		A_cor = torch.zeros(n_batch, 4*4*N//2, 8*N//2, device=device)
		b_cor = torch.zeros(n_batch, 4*4*N//2, device=device)
		for i in range(N//2):
      ...
      #  X = BB * U + AA * x0 + gg
		...
		# single batch 
		Cx = self.Cx_ @ BB
		lx = self.lx_ - self.Cx_ @ AA @ x0 - self.Cx_ @ gg
		ux = self.ux_ - self.Cx_ @ AA @ x0 - self.Cx_ @ gg
		'''
		...
		# Gx <= h
		G = torch.zeros(n_batch, 2 * n_ineq * N , m * N).to(device)
		H = torch.zeros(n_batch, 2 * n_ineq * N, 1).to(device)
		G[:, :n_ineq*N] = A
		G[:, n_ineq*N:] = -A
		H[:, :n_ineq*N] = u
		H[:, n_ineq*N:] = -l
		...
		# A_Cor * [ C_cor * (BB * U + AA * x0 + gg) + d_cor ] < b_cor
		G_cor = torch.bmm(torch.bmm(A_cor, C_cor), BB)
		l_cor = - torch.bmm(torch.bmm(C_cor, AA), x0) - torch.bmm(C_cor, gg) - d_cor
		H_cor = torch.bmm(A_cor, l_cor).squeeze(2) + b_cor
		G = torch.cat([G, G_cor], dim=-2)
		H = torch.cat([H, H_cor], dim=-1)
		# Ax = b, no equality constraints
		n_eq = 0
		...
		'''
		# single batch 
		Q = BB.T @ self.Qx_ @ BB # + self.Ru_  # minimum control R
		p = BB.T @ self.Qx_.T @ (AA @ x0 + gg) + BB.T @ qx
		p = p.squeeze(1)
		'''
		...
		Q = torch.bmm(torch.bmm(BB_T, Qx_), BB) + Ru # try to make Q Semidefinite by adding Ru
		p1 = torch.bmm(AA, x0) + gg
		p = torch.bmm(torch.bmm(BB_T, Qx_), p1) + torch.bmm(BB_T, qx) 
		p = p.squeeze(2)
		#### this seems dangerous, but i found Q have asymmetry
		...
		# 	# grads = torch.autograd.grad(outputs=sol_u.sum(), inputs=[tracking_traj, corridor_rect], retain_graph=True, allow_unused=True)
		# 	grads = torch.autograd.grad(outputs=sol_u.sum(), inputs=[tracking_traj], retain_graph=True, allow_unused=True)
		# 	print(grads)
		# 	for grad in  grads:
		# 		if grad is not None and torch.isinf(grad).any():
		# 			raise ValueError("Infinite values found in gradient.")
		# sol_u [n_batch, nz]
		'''
		# single batch
		sol_u = sol_u.transpose(0, 1) #TODO this is for n_batch = 1
		sol_x = BB @ sol_u + AA @ x0 + gg
		sol_x = sol_x.reshape(N, n)
		zeros = torch.zeros(1, n)
		sol_x = torch.cat((zeros, sol_x), dim=0)
		sol_x = sol_x[1:, :2] - sol_x[:-1, :2]  # step format
		'''
