...
from projects.DifferentiableLQR.qpth.util import get_sizes, bdiag


def lu_hack(x):
    # data, pivots = torch.linalg.lu_factor(x, pivot=not x.is_cuda)
    # torch.linalg.lu_factor is not supported for torch 1.9.1
    data, pivots = torch.lu(x)
...
def solve_kkt(Q_LU, d, G, A, S_LU, rx, rs, rz, ry):
    ...
    invQ_rx = torch.lu_solve(rx.unsqueeze(2), *Q_LU).squeeze(2)  # torch 1.9.1
    ...
    w = torch.lu_solve(-h.unsqueeze(2), *S_LU).squeeze(2)  # torch 1.9.1
    ...
    dx = torch.lu_solve(g1.unsqueeze(2), *Q_LU).squeeze(2) # torch 1.9.1
...
""")
    ...
    G_invQ_GT = torch.bmm(
        G,
        torch.lu_solve(G.transpose(1, 2), *Q_LU))
        # torch 1.9.1 support
